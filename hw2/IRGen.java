...
public class IRGen {
  ...
  private static ClassInfo createClassInfo(Ast.ClassDecl n) throws Exception {
      ...
      return cinfo; // TODO: I think this is okay.
  }
...
//      List<String> localps = new ArrayList<String>();
...
//          }
  ...
  static IR.Func gen(Ast.MethodDecl n, ClassInfo cinfo) throws Exception {
      ...
      if (!cinfo.isMainClass) {
          ...
          IR.Label glbllbl = new IR.Label(cinfo.name + "_" + n.nm);
  ...
  // TODO: Test: [gen(VarDecl, ClassInfo, Env)]
  private static List<IR.Inst> gen(Ast.VarDecl n, ClassInfo cinfo, 
				    Env env) throws Exception {
      if ( /*n.init !exists*/ false ) {
          return null;
      }
      List<IR.Inst> SBR71A = new ArrayList<IR.Inst>();
      IR.Id KF1G = new IR.Id(n.nm);
      CodePack cc598 = gen(n.init, cinfo, env);
      IR.Move mvEC2 = new IR.Move((IR.Dest) KF1G, cc598.src);
      SBR71A.addAll(cc598.code);
      SBR71A.add(mvEC2);
  ...
  static List<IR.Inst> gen(Ast.Print n, ClassInfo cinfo, Env env) throws Exception {
      ...
      if (n.arg == null) {
          ...
          sources.add(p.src);
          code.addAll(p.code);
          code.add(new IR.Call(new IR.Id("print"), true, sources)); // TODO: Wrong-ish?
          //code.add(new IR.Call("print"));
      } else if (n.arg instanceof Ast.StrLit) {
          // 2. If arg is StrLit, generate an IR.Call with "printStr"
          CodePack p = gen(n.arg, cinfo, env);
          sources.add(p.src);
          code.addAll(p.code);
          code.add(new IR.Call(new IR.Id("printStr"), true, sources));
          ...
          if (p.type instanceof Ast.IntType) {
              code.add(new IR.Call(new IR.Id("printInt"), true, sources));
          } else if (p.type instanceof Ast.BoolType) {
              code.add(new IR.Call(new IR.Id("printBool"), true, sources));
          }
  ...
  static CodePack gen(Ast.Id n, ClassInfo cinfo, Env env) throws Exception {
      if (env.containsValue(n)) {
          return new CodePack(cinfo.fieldType(n.nm), null);
      }


    //    TODO: This isn't right, obviously.
      return null;
